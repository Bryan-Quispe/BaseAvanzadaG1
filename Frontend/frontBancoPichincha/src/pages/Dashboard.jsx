...
function Dashboard() {
  ...
  const getTarjetasPorCuenta = async (cuenta_id) => {
    ...
    try {
      const [creditoRes, debitoRes] = await Promise.all([
        fetch(`https://baseavanzadag1.onrender.com/cuentas/${cuenta_id}/tarjetas-credito`, { method: "GET", headers }),
        fetch(`https://baseavanzadag1.onrender.com/cuentas/${cuenta_id}/tarjetas-debito`, { method: "GET", headers }),
      ]);

      const tarjetasCredito = await (creditoRes.ok ? creditoRes.json() : []);
      const tarjetasDebito = await (debitoRes.ok ? debitoRes.json() : []);

      return [...tarjetasCredito.map(t => ({ ...t, tipo: "Crédito" })), ...tarjetasDebito.map(t => ({ ...t, tipo: "Débito" }))];
    } catch (error) {
      console.error(`Error obteniendo tarjetas de la cuenta ${cuenta_id}:`, error);
      return [];
    }
  };
  ...
  useEffect(() => {
    const totalTarjetas = Object.values(tarjetas).reduce((acc, arr) => acc + arr.length, 0);
                ...
                return (
                  <React.Fragment key={cuenta.cuenta_id}>
                    ...
                    {tarjetasCuenta.map((tarjeta, idx) => {
                      ...
                      return (
                        <div key={tarjeta.tarjeta_id} className="relative p-6 rounded-lg shadow border border-purple-400" style={{
                          backgroundImage: `linear-gradient(rgba(255,255,255,0), rgba(255,255,255,0)), url('${tarjeta.tarjeta_estilo == "VISA" ? "/fondoGris.svg" : "/fondoDorado.svg"}')`,
